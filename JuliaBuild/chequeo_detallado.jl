...
const VMNV_PATH = let path = Sys.which("vmnv")
...
make_entry(lhs, rhs, expr, desc) = Dict(
    "ok" => lhs == rhs,
    "lhs" => string(lhs),
    "rhs" => string(rhs),
    "expression" => expr,
...
function run_vmnv_testvectors(dataset::AbstractString)
...
function obtain_testvectors(dataset::AbstractString, ::Type{G}) where {G}
    output = run_vmnv_testvectors(dataset)
    lines = split(output, '\n')
...
function detailed_chequeo(dataset::AbstractString)
    ...
    testvectors = obtain_testvectors(dataset, typeof(proposition.g))
    ρ = testvectors.ρ
...
function main()
    dataset = length(ARGS) >= 1 ? normpath(abspath(ARGS[1])) : DEFAULT_DATASET
    result = detailed_chequeo(dataset)
    ...
    output_path = joinpath(SCRIPT_DIR, "chequeo_detallado_result.json")
